...
lasso_selected_vars = lasso_coef_means$variable[lasso_coef_means$mean != 0]
...
cat("\nComparison to LASSO:\n")
cat("LASSO R²:", round(mean(results_r2$r2_scores), 4), "\n")
cat("RF R²:   ", round(rf_single$r.squared, 4), "\n")
cat("RF improvement:", round(rf_single$r.squared - rf_single$r.squared, 4), "\n\n")
...
lasso_coeffs_clean = lasso_coef_means %>%
  select(variable, lasso_coeff = mean)

lasso_coeffs_clean = lasso_coef_means %>%
   select(variable, lasso_coeff = mean)
  lasso_rf_comparison = lasso_coeffs_clean %>%
  full_join(imp_stability %>% select(variable, rf_importance = mean_imp), 
          by = "variable") %>%
  mutate(
...
perf_comparison = data.frame(
  ...
  R_squared = c(rf_single$r.squared, rf_single$r.squared, mean(r2_scores_rf)),
